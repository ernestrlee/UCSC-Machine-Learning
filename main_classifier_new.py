...
for imageIndex in range(5):
    ...
    plt.imshow(Xtrain[np.random.randint(0,Xtrain.shape[0],1),:].reshape(112, 92), cmap=plt.cm.gray)
    plt.grid(False);
    plt.xticks([]);
    plt.yticks([]);
    plt.title('Face: %i\n' % imageIndex, fontsize=10)
...
y_prob = log_reg.predict_proba(Xtest);
print("y_prob.shape",y_prob.shape);
print(y_prob);

#performance
#score = log_reg.score(XTest_reduced,TtestGood);
score = log_reg.score(Xtest,TtestGood);
print("Score \n",score);
...
for plotIndex, badIndex in enumerate(misclassifiedIndexes[0:5]):
    ...
    plt.imshow(np.reshape(Xtest[badIndex], (112,92)), cmap=plt.cm.gray)
